...
* 4 rows named row1, row2, row3 and row4
...
<configuration>
    ...
    <version>3.0</version>
    <creation>1430:2015Feb28 by A. BC</creation>
    ...
    <junction>
        ...
        <junction>
            <type>row</type>
            <!—- The rows are called (row1|row2|row3|row4) -->
            <name>"row[1:1-4]"</name>
            <controller>
                <!—- This controller takes its name from its row’s name -->
                <!—- It is assumed that the row’s name is IP resolvable -->
                <host>@</host>
                <port>55815</port>
                <mca-params>sensor_base_sample_rate=5</mca-params>
                <mca-params>sensor_heartbeat_rate=10</mca-params>
            </controller>
            <junction>
                <type>rack</type>
                <!—- The racks have a compounded name, a part comes from its row,-->
                <!—- a part comes from the regex below (=rack001,…,=rack128)  -->
                <!—- So a rack’s name will be, for example, row2=rack111  -->
                <name>@=rack[3:1-128]</name>
                <controller>
                    <host>@</host>
                    <port>55815</port>
                    <mca-params>sensor_base_sample_rate=5</mca-params>
                ...
                <junction>
                    <type>node</type>
                    <!—- Again because of the use of @, a node will have compounded name  -->
                    <!—- One node will have, for example, the name row2=rack111=node79 -->
                    <name>"@=node[2:1-80]"</name>
                    <controller>
                        <host>@</host>
                        <port>55815</port>
                        <mca-params>sensor_base_sample_rate=5</mca-params>
    ...
    <scheduler>
        <!—shost identifies the node that houses the ORCM scheduler. Only one allowed at this time -->
        <shost>localhost</shost>
        <!-- Port used by orcm scheduler -->
        <port>55820</port>
        <!-- Each queue entry is in the form of queueName:priority:list-of-nodes, where the list-of-nodes
             is a comma-separated list of node ranges in a regex. The "default" queue is automatically created
             and will contain all nodes not explicitly contained in any other defined queue.
             The default queue has a zero priority.  -->
        <queues>"quick:100:a[4:1-200]"</queues>
        <queues>"slow:20:b[4:500-512]"</queues>
    </scheduler>
</configuration>
```