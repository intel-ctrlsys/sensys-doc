...
* ORCM Regex                     https://github.com/orcmteam/gemeter/wiki/3.3-ORCM-Regex 
...
of logical hostname and port number. For example, if the user/admin wants to run 4 aggregators on the same
node, he/she can define the logical hostnames for the node as follows:

Add the following logical hostnames: "agg01", "agg02", "agg03", "agg04" into the /etc/hosts file
to the node ip mapping with each one corresponding to one aggregator. In addition, different port
numbers need to be given for these aggregators.
...
To run multiple aggregators on the same node, each aggregator needs its port number as follows:
...
In addition to running multiple aggregators on the same node, ORCM also supports running multiple
compute deamons on the same compute node. The configuration is similar with the example shown above
with different port numbers for different compute daemons.
