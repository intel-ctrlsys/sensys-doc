Admin-focused tool for interacting with ORCM.  This tool has the ability to run as an interactive shell or as a single one-shot command.  Currently the tool provides information about configured resources, sensors and, allows to modify the workflows of ORCM.
...
Notice in the example, control is returned to the user to complete the command as desired.
