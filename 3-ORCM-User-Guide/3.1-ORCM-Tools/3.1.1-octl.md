...
nothing to do with the runningstatus of the nodes, nor will it change the status.
...
This command only brings the nodes into the available resource pool of the scheduler. It has nothing to do with the running status of the nodes, nor will it change the status.
...
* LOG:
    - SYNTAX:
```sh
        query log <search word> [start-date start-time end-date end-time] <nodelist>
```
