...
After using the above command, user will be able to remove a workflow running in the target. Here, wild-card character * is allowed for workflow_name and workflow_id. Remember that if * is used for workflow_name,
...
Output:
    None
```
__NOTE__: This command is hardware-dependent and it might not be supported on all platforms. Also, due to limitations
in the BMC and/or the ipmiutil driver, this sensor plugin may interfere with the `ipmi` and `nodepower` sensor plugins.
