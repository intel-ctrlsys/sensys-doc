...
The current implementation displays the node connection state: either up(U), down(D), or unknown(?) and the job state: allocated or unallocated.  The node specification is an ORCM node regex.
...
Sample workflow XML files can be found at section 1-ORCM/1.8-Data-Smoothing-Algorithms-Analytics.
...
#### 3.1.1.10 Sensor Sample Rate Setting
...
#### 3.1.1.11 Sensor Sampling Control
...
#### 3.1.1.12 Sensor Storage Control
...
* ORCM nodes regular expression: Example: host[2:1-3] to specify every node.
* ORCM nodes with wildcards: Example: host* to specify every node.
...
Or:
    ...
    Failure information
```
__NOTE__: This command is hardware-dependent and it might not be supported on all platforms. Also, due to limitations in the BMC and/or the ipmiutil driver, this sensor plugin may interfere with the `ipmi` and `nodepower` sensor plugins.

__NOTE__: Both __enable__ and __disable__ subcommands perform write operations. It is importat to configure BMC access with the proper privilege level, otherwise the chassis-id feature might not be able to change the LED state.
