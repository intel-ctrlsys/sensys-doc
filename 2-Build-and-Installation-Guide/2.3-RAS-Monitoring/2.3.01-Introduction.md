...
This component is used to read variables from SNMP devices. A configuration file is needed as input (see `sensor_snmp_configfile` MCA parameter below). 
...
* `sensor_snmp_configfile`: User can specify the configuration file for snmp devices. If such a variable is not specified it defaults to `<orcm/install/path>/etc/snmp.xml`

Configuration file sections:
The overall configuration section is called `snmp`, and the respective tag should be available in the configuration file.
...
The file `snmp.xml` used for connecting to network devices using SNMP
