...
* Ipmi and nodepower sensors cannot be executed simultaneously on different threads.
* Ipmi and nodepower sensors may interfere with the OCTL command `chassis-id`.

__NOTE__: The ipmi sensor plugin takes a long time to perform inventory and data collection, and this scales with the number of BMCs to be sampled. In order to avoid a bottleneck, ipmi sensor should be run in a different thread than the other sensors (with the exception of nodepower sensor as mentioned above).
...
__NOTE__: Due to limitations in the BMC and/or the ipmiutil driver, the following conditions apply:
* Nodepower and ipmi sensors cannot be executed simultaneously on different threads. The ipmi sensor plugin takes a long time to perform inventory and data collection, and this scales with the number of BMCs to be sampled; this bottleneck might affect nodepower sampling.
* Nodepower and ipmi sensors may interfere with the OCTL command `chassis-id`.
