...
NOTE: Due to limitations in the BMC and/or the ipmiutil driver, this sensor plugin may not be used with the nodepower sensor plugin.

MCA parameters:

* **sensor\_ipmi\_bmc\_username**: Used to set the username of the remote BMC nodes for retrieving the metrics via the IPMI interface
* **sensor\_ipmi\_bmc\_password**: Used to set the password of the remote BMC nodes for retrieving the metrics via the IPMI interface, for the above configured username
...
NOTE: Due to limitations in the BMC and/or the ipmiutil driver, this sensor plugin may not be used with the ipmi sensor plugin.
...
* Execute OCTL to activate the workflow text file which we created, i.e.: # <orcm_install_path>/bin/octl analytics workflow add <path_to_workflow_text_file>

Additionally to that, the syslog sensor needs a workflow text file with the following content:
nodelist:<node_list>
component:filter:args:data_group=syslog
component:syslog:args:msg_regex=access granted,severity=6,facility=10

* nodelist = specifies the list of nodes in which the workflow will be tracked.
* component:filter:args:sensor = a sensor list which will interact with the analytics framework, this list is separated by a semicolon. i.e. “syslog;freq;coretemp”
* component:syslog:args = These are the arguments set for a given sensor. Arguments can change depending on which sensor is specified, for syslog sensor you can specify the following arguments:
...
* facility: Message facility which has to be met in order to send it to notification framework.
...
This component is used to read variables from SNMP devices. A confguration file is needed as input (see sensor_snmp_configfile MCA parameter below). 
...
* sensor_snmp_configfile: User can specify the configuration file for snmp devices. If such a variable is not specified it defaults to <orcm/install/path>/etc/snmp.conf

Configuration file sections:
* <nodes> used for specifying what devices should every aggregator node query. Every line in this section is of the form:
<aggregator>=<node1>,<node2>,<regex node>,<logical group>
* <snmp-device> section(s), where the following fields are required:
    * SNMPv1 Devices:
        * version=1
        * hostname, is the device hostname or ip address
        * user, is the community user of the device (that’s the name)
    ...
    * SNMPv3 Devices:
        ...
        * auth, is the encryption mechanism. This can be MD5 or SHA1
        * sec, is the security access method. This can be NOAUTH, AUTHNOPRIV, AUTHPRIV
        * oids, is the comma separated list of oids to query. Both numerical OIDs and textual MIB names are supported.
...
```
<nodes>
localhost=server05,server[2:1-10]

<nodes>
node01=$group1,switch20,switch[2:0-10]

<snmp-config>
hostname=server[2:0-20],server21
version=3
user=user
pass=12345678
auth=MD5
sec=AUTHPRIV
oids=1.3.6.1.4.1.343.1.1.3.1,1.3.6.1.4.1.343.1.1.3.4

<snmp-config>
hostname=switches[2:0-20],switch21
version=1
user=user
location=X Lab
oids=SNMPv2-MIB::sysServices.0, SNMPv2-MIB::sysORDescr.1
```
Security Advisory:
==================
