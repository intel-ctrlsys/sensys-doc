...
__NOTE__: Due to limitations in the BMC and/or the ipmiutil driver, this sensor plugin should not be used with the `ipmi` sensor plugin, if so aggregators won't be able to properly retrieve data but compute nodes will.
Also, due to the same limitation, the OCTL command `chassis-id` may interfere with this sensor as well.
