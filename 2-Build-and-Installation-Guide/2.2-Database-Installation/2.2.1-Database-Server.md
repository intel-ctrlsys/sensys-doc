...
| -------------------- | --------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| PostgreSQL Server    | 9.3 or higher   | Yes  | Required on the server that will<br>contain the database                                                                                                                                                  |
| PostgreSQL Client    | 9.3 or higher   | Yes  | It’s required during database<br>installation and may also be<br>installed on certain client machines<br>for basic database management tasks:<br>testing the database connection,<br>data and schema management, etc. |

####2.2.1.2 Installation Overview

<br>At a high level, installing the database requires the following steps:
...
3. Enable external TCP connections to the _postgresql_ service
    ...
    * Enable networking for PostgreSQL
        ...
        * Edit the following line to specify what IP addresses to listen on:
            * <code>listen_addresses = ‘&lt;comma-separated list of addresses&gt;’</code>
            * NOTE: use <code>‘*’</code> to specify all
4. Create _orcmuser_
    * Use the _createuser_ command as the default _postgres_ user:
        * <code>sudo –u postgres createuser –P orcmuser</code>
...
5. Create the _orcmdb_ database
    ...
    * Create the database:
        * <code>sudo –u postgres createdb --owner orcmuser orcmdb</code>
    * Use the _psql_ tool to run the database creation script:
        * <code>psql --username=orcmuser --dbname=orcmdb --password –f orcmdb_psql.sql</code>
...
6. Verify the installation
    * Connect to the database from a remote machine:
        * <code>psql –-host=<hostname or IP address> -–username=orcmuser –-dbname=orcmdb --password</code>
