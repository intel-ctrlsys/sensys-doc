...
Edit the `log_destination` value in the `postgresql.conf` file to include `syslog`.  The default value for the `log_destination` is `stderr`.  To see current value of log destination, connect to the database and run `show log_destination;`.  
