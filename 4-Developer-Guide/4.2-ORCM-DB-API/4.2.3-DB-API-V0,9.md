The purpose of the DB API is to help abstract the schema from applications (including ORCM).  This is achieved through functions (or stored procedures) and views.  These functions and views are accessible via standard SQL interfaces.  So applications can connect to the database via they're preferred interface (e.g. ODBC, the DBMS client API, etc.) and make the desired calls to store and query data.  This helps minimize the impact to applications whenever the schema needs to change, as they don't necessarily need to know the details of the tables and they're relations.
