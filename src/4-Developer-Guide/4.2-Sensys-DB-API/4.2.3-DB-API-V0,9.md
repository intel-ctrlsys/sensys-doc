The purpose of the DB API is to help abstract the schema from applications (including Sensys).  This is achieved through functions (or stored procedures) and views.  These functions and views are accessible via standard SQL interfaces.  So applications can connect to the database via they're preferred interface (e.g. ODBC, the DBMS client API, etc.) and make the desired calls to store and query data.  This helps minimize the impact to applications whenever the schema needs to change, as they don't necessarily need to know the details of the tables and they're relations.
...
* Parameters:
    ...
    * data_type_id: the data type ID (as defined by the application).
...
```
record_diag_test_result(hostname varchar,
                        diag_type varchar,
...
* Parameters:
    ...
    * data_type_id*: a data type ID assigned by the application.
...
* Synopsis: provides a view of all the data samples that have been collected.
* Columns:
    * hostname
    * data_item: the name of the data item.
    * data_type_id: the application-assigned data type ID.
    * time_stamp: the time stamp at which the sample was collected.
...
* Synopsis: provides a view of all the node features that have been set for all nodes.  Further filtering is necessary to retrieve the features of a particular node.
* Columns:
    * node_id: the node ID as assigned to it in the database.
    * hostname
    * feature_id: the feature ID as assigned to it in the database.
