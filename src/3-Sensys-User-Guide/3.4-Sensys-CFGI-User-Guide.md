...
```
You can find a full workflow documentation and examples in section 3.9-Data-Smoothing-Algorithms-Analytics
...
* ```<data_group> = {componentpower|coretemp|dmidata|errcounts|freq|ipmi|
     mcedata|nodepower|resusage|sigar|snmp|syslog|udsensors}``` : Used by **filter**. This is a sensor name from which the data will be filtered.
